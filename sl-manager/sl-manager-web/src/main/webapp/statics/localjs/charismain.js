$(document).ready(function() {
	...
	for (var i = 0; i < json.length; i++) {
		...
		//config main menu
		result = result + '<li class="nav-header hidden-tablet" onclick="$(\'#test' + i + '\').toggle(500);" style="cursor:pointer;">' + json[i].level1.authorityname + '</li>';
		//config sub menus
		result = result + "<li><ul class=\"nav nav-tabs nav-stacked\" id=\"test" + i + "\">";

		for (var j = 0; j < json[i].level2.length; j++) {
			var pic;
			switch (j) {
			case 0:
				pic = "icon-home";
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\" href=\"" + json[i].level2[j].url + "\"><i class=" + pic + "></i><span class=\"hidden-tablet\">" + json[i].level2[j].authorityname + "</span></a></li>";
		}
		result = result + "</ul></li>";
	}
	$("#menus").append(result);
