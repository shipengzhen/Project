...
$('.adduser').click(function(e){
	$("#add_formtip").html('');		//错误列表清空
	e.preventDefault();
	$('#addUserDiv').modal('show');		//显示模态框
		...
		success: function(result){
			...
			if("failed" == result){
				...
				if(roleId == '2'){
					...
					$.post("admin/findRoleType",{'roleId':roleId},function(result){
						if(result != ""){
							for(var i=0;i<result.length;i++){
								if(result[i].valueId == userType){
									$("#m_selectusertype").append("<option value=\""+userType+"\" selected=\"selected\">"+userTypeName+"</option>");
								}else{
									$("#m_selectusertype").append("<option value=\""+result[i].id+"\">"+result[i].roletypename+"</option>");
								}
							}
						}else{
							alert("用户类型加载失败！");
				...
				var isstart = m.status;
				if(isstart == '0'){
					$("#m_isstart").append("<option value=\"0\" selected=\"selected\">启用</option><option value=\"1\">不启用</option>");
				}else{
					$("#m_isstart").append("<option value=\"0\">启用</option><option value=\"1\" selected=\"selected\">不启用</option>");
				}
...
$('.deluser').click(function(e){
	...
	if(confirm("您确定要删除【"+d_logincode+"】这个用户吗？")){
		//delete
		$.post("admin/delteUser",{'delId':d_id,'delIdCardPicPath':d_idcardpicpath,'delBankPicPath':d_bankpicpath},function(result){
			if("success" == result){
				alert("删除成功！");
...
$("#m_roleId").change(function(){
	...
	if(sel_role == 2){
		$.post("admin/findRoleType",{'roleId':sel_role},function(result){
			if(result != ""){
				for(var i=0;i<result.length;i++){
					$("#m_selectusertype").append("<option value=\""+result[i].id+"\">"+result[i].roletypename+"</option>");
				}
...
$("#selectrole").change(function(){
	...
	$("#selectusertype").empty();		//删除 本元素的子标签
	//$("#selectusertype").append("<option value=\"\" selected=\"selected\">--请选择--</option>");	//追加
	...
	if(sel_role == 2){
		$.post("admin/findRoleType",{'roleId':sel_role},function(result){
			if(result != ""||result != null){
				...
				for(var i=0;i<result.length;i++){
					$("#selectusertype").append("<option value=\""+result[i].id+"\">"+result[i].roletypename+"</option>");
				}
			}else{
				alert("用户类型加载失败！");
			}
...
$("#a_logincode").blur(function(){
	...
	if(alc != ""&&alc.length>6){
		$.post("admin/doUserByName",{'loginCode':alc,'id':'-1'},function(result){
			if(result == "repeat"){
...
$("#a_email").blur(function(){
	var flag = checkEmail($("#a_email").val());		//正则表达式
	if(flag == false){
		$("#add_formtip").css("color","red");
...
function addUserFunction(){
	...
	if($.trim($("#a_logincode").val()) == "" || $("#a_logincode").val() == null){
		...
		result = false;
	}else if($("#a_logincode").val().length<6||$("#a_logincode").val().length>12){
		$("#add_formtip").css("color","red");
		$("#add_formtip").append("<li>名称只能在 6~12位之间</li>");
		result = false;
...
//			0---a_idPic--a_uploadbtnID--statics/uploadfilesj1202890398_Personal.jpg--a_fileInputIDPath---a_fileInputID
	//delete
