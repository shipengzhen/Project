$(document).ready(function() {
	...
	for (var i = 0; i < json.length; i++) {
		...
		//config main menu
		result = result + '<li class="nav-header hidden-tablet" onclick="$(\'#test' + i + '\').toggle(500);" style="cursor:pointer;">' + json[i].sysAuthority.authorityname + '</li>';
		//config sub menus
		...
		for (var j = 0; j < json[i].menus.length; j++) {
			...
			result = result + "<li><a class=\"ajax-link\" style=\"cursor:pointer;\" href=\"" + json[i].menus[j].sysAuthority.url + "\"><i class=" + pic + "></i><span class=\"hidden-tablet\">" + json[i].menus[j].sysAuthority.authorityname + "</span></a></li>";
		}
		result = result + "</ul></li>";
	}
	$("#menus").append(result);
